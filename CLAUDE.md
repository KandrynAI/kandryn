...
| `suggestion_files` | `id`, `suggestion_id` FK (cascade), `seq`, `op` (`create`/`edit`/`delete`), `file_path`, `content`, `hunks` jsonb, `resolved`, `apply_status` (`pending`/`applied`/`failed`), `apply_error`, `lines_added`, `lines_removed` (0022). **Unique (suggestion_id, seq)**. Phase 0: exactly one row per suggestion. Read via `loadSuggestionFiles`/`suggestionPrimaryFile` (`services/suggestionFiles.ts`) |
