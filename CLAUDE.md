...
| `suggestion_files` | `id`, `suggestion_id` FK (cascade), `seq`, `op` (`create`/`edit`/`delete`), `file_path`, `content` (resolved content for create/edit), `hunks` jsonb (edit search/replace pairs, 0023), `source_blob_sha` (source read at generation, for staleness, 0023), `resolved`, `apply_status` (`pending`/`applied`/`failed`), `apply_error`, `lines_added`, `lines_removed` (0022). **Unique (suggestion_id, seq)**. Generation emits one file. Read via `loadSuggestionFiles`/`suggestionPrimaryFile` (`services/suggestionFiles.ts`) |
