...
export interface AegisTicketInput {
  ...
  /**
   * The originating work item's PLM key (e.g. "PAY-214"), when there is one.
   * A baseline scan has no work item — the finding is in code that predates any
   * run — so its ticket is filed top-level. `createSecurityFindingItem` already
   * accepts a null parent; only this type insisted on one.
   */
  parentExternalId?: string | null;
  /** Context line in the ticket body. Falls back to a description of the source. */
  parentTitle?: string;
...
function buildFindingBody(finding: AegisFinding, parentTitle: string | undefined): string {
  return [
    ...
    "",
    parentTitle ? `Parent: ${parentTitle}` : "Source: baseline scan of the existing codebase",
